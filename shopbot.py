...
async def safe_send(ctx_or_channel, content=None, **kwargs):
    """ส่งข้อความแบบปลอดภัยโดยจัดการ Cloudflare Rate Limit"""
    try:
        ...
        if e.status == 429:
            error_msg = str(e)
            if "cloudflare" in error_msg.lower() or "1015" in error_msg:
                print(f"🔒 Cloudflare Rate Limit (Error 1015) - บอททำงานแบบ silent mode")
            else:
                print(f"⚠️ Discord Rate Limit - รอ rate limit หมด")
        else:
...
async def on_ready():
    ...
    if is_render:
        print("✅ Production: Discord connection successful on Render.com")
    else:
        print("✅ Development: Discord connection successful locally")
...
async def shop(ctx, ประเทศหรือหมวด: str = None, หมวด: str = None):
    ...
    if not products:
        await safe_send(ctx, f"❌ ไม่มีสินค้าในประเทศ `{COUNTRY_NAMES[country]}` หมวด `{CATEGORY_NAMES[category]}`")
        return
...
async def on_command_error(ctx, error):
    ...
    try:
        ...
        if rate_error.status == 429:
            # ตรวจสอบว่าเป็น Cloudflare Rate Limit หรือไม่
            error_msg = str(rate_error)
            if "cloudflare" in error_msg.lower() or "1015" in error_msg:
                print(f"🔒 Cloudflare Rate Limit detected (Error 1015) - บอทจะทำงานแบบ silent mode")
            else:
                print(f"⚠️ Discord Rate Limit - cannot send error message: {rate_error}")
        else:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if is_render:
            ...
            print("🛠️ Development Mode: Local environment")
        
        if is_render:
            # สำหรับ Render.com: เพิ่มเวลารอพอประมาณเพื่อหลีกเลี่ยง Cloudflare Rate Limit
            delay = random.uniform(15, 25)  # ลดเวลารอเป็น 15-25 วินาที
            ...
            # Loop เพื่อลองเชื่อมต่อใหม่หาก Rate Limited
            max_retries = 3
            for attempt in range(max_retries):
                try:
                    bot.run(TOKEN, reconnect=False)
                    break  # หากเชื่อมต่อสำเร็จ ออกจาก loop
                except discord.HTTPException as retry_e:
                    if retry_e.status == 429 and attempt < max_retries - 1:
                        wait_time = random.uniform(60, 120)
                        print(f"🔄 Attempt {attempt + 1}/{max_retries} - Rate Limited, รอ {wait_time:.1f} วินาที แล้วลองใหม่...")
                        time.sleep(wait_time)
                    else:
                        raise  # Re-raise หากไม่ใช่ rate limit หรือลองครบแล้ว
        else:
        ...
        if e.status == 401:
            ...
            if is_render:
                # สำหรับ Render: รอพอประมาณเพื่อลด Cloudflare Rate Limit
                delay = random.uniform(120, 180)  # รอ 2-3 นาที แทน 10 นาที
                print(f"🔄 Production: ตรวจพบ Cloudflare Rate Limit - รอ {delay:.1f} วินาที ก่อน restart...")
                time.sleep(delay)
            else:
